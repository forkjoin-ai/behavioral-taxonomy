...
function normalizeEmotion(raw: RawEmotionRecord, level: 'primary' | 'secondary' | 'tertiary'): Emotion {
    return {
        id: raw.id,
...
export function getBiases(): CognitiveBias[] {
    ...
    return Object.entries(data)
        .filter(([key, value]) => key !== '_metadata' && value != null && typeof value === 'object' && 'id' in (value as Record<string, unknown>))
...
export function getStatistics(): {
    ...
    const avgDifficulty = allLoops.length > 0
        ? allLoops.reduce((sum, loop) => sum + loop.intervention.difficulty, 0) / allLoops.length
        : 0;
